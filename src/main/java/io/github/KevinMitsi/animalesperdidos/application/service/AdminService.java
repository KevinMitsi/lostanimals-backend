...
public final class AdminService implements AdminUseCase {
    ...
    @Override public CompletionStage<List<ServiceAreaView>> serviceAreas() {
        return areas.list().thenApply(values -> values.stream().map(value -> new ServiceAreaView(value.cityId(),
    ...
    @Override public CompletionStage<Void> setServiceArea(UUID adminId, UUID cityId, boolean enabled) {
        return areas.setEnabled(cityId, enabled, adminId, clock.instant());
    }
    @Override public CompletionStage<Void> changeRole(UUID adminId, UUID userId, UserRole role) {
        if (adminId.equals(userId) && role != UserRole.ADMIN)
            throw new BusinessRuleViolation("An administrator cannot remove their own role");
        return users.findById(userId).thenCompose(optional -> optional.<CompletionStage<Void>>map(user ->
